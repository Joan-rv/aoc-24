...
sum = 0
for code in codes:
    directional_moves_prev = set(numeric_press(code))
    for i in range(2):
        print(i, end='\r')
        directional_moves = set()
        for moves in directional_moves_prev:
            new_moves = directional_press(moves)
    ...
    print(f"{len_min} * {to_numeric(code)}")
    sum += len_min * to_numeric(code)
